...
exprt cast_or_cut(const exprt &expr, const typet& dest_type, std::size_t w){
  ...
  if(type_try_dynamic_cast<bool_typet>(expr.type()) || type_try_dynamic_cast<c_bool_typet>(expr.type())){
    if(type_try_dynamic_cast<bool_typet>(dest_type)){
      ...
      if(u->get_width() <= w){
        return typecast_exprt::conditional_cast(expr, dest_type);
      } else {
        return typecast_exprt::conditional_cast(expr, unsignedbv_typet{w});
      }
    } else if(const auto s = type_try_dynamic_cast<signedbv_typet>(dest_type)){
      if(s->get_width() <= w){
        return typecast_exprt::conditional_cast(expr, dest_type);
      } else {
        return typecast_exprt::conditional_cast(expr, signedbv_typet{w});
      }
    } else
      UNHANDLED_CASE;
    ...
    if(type_try_dynamic_cast<bool_typet>(dest_type)){
      if(expr_short){
        ...
        return typecast_exprt::conditional_cast(make_byte_extract(expr, constant_exprt{"0", unsignedbv_typet{1}}, uw), dest_type);
      }
      ...
      if(!expr_short && !dest_short){
        return typecast_exprt::conditional_cast(make_byte_extract(expr, constant_exprt{"0", unsignedbv_typet{1}}, uw), uw);
      } else if(!expr_short && dest_short) {
        return typecast_exprt::conditional_cast(make_byte_extract(expr, constant_exprt{"0", unsignedbv_typet{1}}, uw), dest_type);
      } else if(expr_short && !dest_short) {
        return typecast_exprt::conditional_cast(expr, uw);
      ...
      if(!expr_short && !dest_short){
        return typecast_exprt::conditional_cast(make_byte_extract(expr, constant_exprt{"0", unsignedbv_typet{1}}, sw), sw);
      } else if(!expr_short && dest_short) {
        return typecast_exprt::conditional_cast(make_byte_extract(expr, constant_exprt{"0", unsignedbv_typet{1}}, sw), dest_type);
      } else if(expr_short && !dest_short) {
        return typecast_exprt::conditional_cast(expr, sw);
      } else{
        return typecast_exprt::conditional_cast(expr, dest_type);
      }
    } else
    ...
    if(type_try_dynamic_cast<bool_typet>(dest_type)){
      ...
      if(!expr_short && !dest_short){
        ...
        return typecast_exprt::conditional_cast(
          make_byte_extract(expr, constant_exprt{"0", unsignedbv_typet{1}}, sw), dest_type);
      } else if(!dest_short) {
        return typecast_exprt::conditional_cast(expr, sw);
...
void goto_convertt::binaryop_between_bools(const exprt &a, const irep_idt op, const exprt &b, const optionalt<exprt> &dest_deref, const optionalt<exprt> &of_deref, const size_t w, goto_programt &dest, const irep_idt &mode){
  ...
  if(dest_deref)
  {
    ...
    } else if (const auto udt = type_try_dynamic_cast<unsignedbv_typet>(dest_deref->type()))
    {
      if(udt->get_width() <= w)
      {
        rhs = typecast_exprt::conditional_cast(res, dest_deref->type());
      }
      else
      {
        rhs = make_byte_update(
          *dest_deref,
          ...
          typecast_exprt::conditional_cast(res, unsignedbv_typet{w}));
      }
    } else if (const auto sdt = type_try_dynamic_cast<signedbv_typet>(dest_deref->type()))
    {
      ...
      else
      {
        rhs = make_byte_update(
          *dest_deref,
          constant_exprt{"0", unsignedbv_typet{1}},
          typecast_exprt::conditional_cast(res, signedbv_typet{w}));
      }
    }
...
void goto_convertt::unaryop_bool(const irep_idt op, const exprt &a, const optionalt<exprt> &dest_deref, const optionalt<exprt> &of_deref, const size_t w, goto_programt &dest, const irep_idt &mode){
  PRECONDITION(a.type().id() == ID_bool);
  if(of_deref)
    convert_assign(code_assignt{*of_deref, constant_exprt{"0", of_deref->type()}}, dest, mode);
  if(dest_deref)
  {
    exprt res;
    if(op == ID_unary_minus)
      res = a;
    else if(op == ID_bitnot)
      res = true_exprt{};
    else
      UNREACHABLE;
...
void goto_convertt::unary_op_int(const irep_idt op, const exprt &a, const optionalt<exprt> &dest_deref, const optionalt<exprt> &of_deref, const size_t w, goto_programt &dest, const irep_idt &mode, const source_locationt& sourceLocation){
  ...
  const bool has_of_defined = op == ID_unary_minus;
  if(!sign_a){
    const auto wtype = std::max(width_a, width_dest);
...
void goto_convertt::remove_assign_bitwidth(
  ...
  const irep_idt &mode){
  clean_expr(expr.dest(), dest, mode, true);
  clean_expr(expr.a(), dest, mode, true);
  const auto &a = expr.a();
  auto &c = expr.dest();
  ...
  if(a.type().id() == ID_bool){
    ...
    rhs_data = extractbit_exprt{a,0};
  } else if (auto a_ibt = type_try_dynamic_cast<integer_bitvector_typet>(a.type())){
    ...
    else
    {
      ...
      rhs_data = typecast_exprt::conditional_cast(
        make_byte_extract(a, constant_exprt{"0", unsignedbv_typet{1}}, tp),
        tp);
    }
  } else {
    UNREACHABLE;
  ...
  if(c.type().id() == ID_bool || c.type().id() == ID_c_bool){
    ...
    auto rhs = (c_ibt->get_width() <= w)?(exprt)typecast_exprt::conditional_cast(rhs_data, lhs_type):rhs_data;//make_byte_update(c, constant_exprt{"0", unsignedbv_typet{1}}, typecast_exprt::conditional_cast(rhs_data, lhs_type));
    auto lhs = (c_ibt->get_width() <= w)?c:make_byte_extract(c, constant_exprt{"0", unsignedbv_typet{1}}, rhs_data.type());
    auto assn = code_assignt{lhs, rhs};
    assn.add_source_location() = expr.source_location();
    convert_assign(assn, dest, mode);
    expr.make_nil();
...
void goto_convertt::remove_nz_bitwidth(
  nz_bitwidtht &expr,
  goto_programt &dest,
  bool result_is_used,
  const irep_idt &mode)
  ...
  const irep_idt &mode)
{
  ...
  if(expr.a().type().id() == ID_bool){
    ...
    if(a_ibt->get_width() <= w) {
      ...
      typet tw = a_ibt->smallest() < 0 ? (typet)signedbv_typet{w} : unsignedbv_typet{w};
      rhs = typecast_exprt::conditional_cast(make_byte_extract(expr.a(), constant_exprt{"0", unsignedbv_typet{1}}, tw), tw);
    }
...
void goto_convertt::remove_unary_bitwidth_overflow(
  ...
  const irep_idt &mode)
{
  ...
  if(!read_dest && !read_of){
    ...
    if(type_a.id() == ID_bool || type_a.id() == ID_c_bool){
      const auto expr_a = cast_or_cut(expr.a(), bool_typet{}, w);
      unaryop_bool(expr.op(), expr_a, dest_deref, of_deref, expr.width(), dest, mode);
      expr.make_nil();
      return;
    } else {
  ...
  bool address_taken)
{
  ...
  else if(statement == ID_unary_bitwidth_overflow)
  {
    remove_unary_bitwidth_overflow(
      to_unary_bitwidth_overflow(expr), dest, result_is_used, mode);
  }
  else if(statement == ID_assign_bitwidth)
  {
    remove_assign_bitwidth(
      to_assign_bitwidth(expr), dest, result_is_used, mode);
  }
